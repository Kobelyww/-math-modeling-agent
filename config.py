...
def load_settings(env_path: str | Path | None = None) -> Settings:
    ...
    if llm_provider not in {"deepseek", "mimo"}:
        raise RuntimeError(f"Unsupported LLM_PROVIDER: {llm_provider}")

    provider_prefix = "MIMO" if llm_provider == "mimo" else "DEEPSEEK"
    api_key = os.getenv(f"{provider_prefix}_API_KEY")
    if not api_key:
        raise RuntimeError(f"Missing {provider_prefix}_API_KEY in .env")

    temperature = float(os.getenv("DEEPSEEK_TEMPERATURE", "0.3"))
    raw_model = os.getenv(f"{provider_prefix}_MODEL", "deepseek-v4-pro")
